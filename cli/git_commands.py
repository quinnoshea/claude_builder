...
from ..utils.git import GitIntegrationManager, GitHookManager, GitBackupManager
from ..utils.exceptions import ClaudeBuilderError, GitError
...
def status(project_path: str):
    ...
    try:
        ...
        if commit_msg_hook.exists():
            with open(commit_msg_hook, 'r', encoding='utf-8') as f:
                content = f.read()
...
def exclude(project_path: str, force: bool):
    ...
    try:
        ...
        if result.success:
            ...
            raise click.ClickException("Exclude setup failed")
        
    except Exception as e:
        console.print(f"[red]Error setting up excludes: {e}[/red]")
        raise click.ClickException(f"Failed to setup excludes: {e}")
...
def unexclude(project_path: str):
    ...
    try:
        ...
        if result.success:
            ...
            for error in result.errors:
                console.print(f"  • {error}")
            raise click.ClickException("Unexclude operation failed")
        
    except Exception as e:
        console.print(f"[red]Error removing excludes: {e}[/red]")
        raise click.ClickException(f"Failed to remove excludes: {e}")
...
def install_hooks(project_path: str, policy: Optional[str], pre_commit: bool):
    ...
    try:
        ...
        if policy:
            from ..core.models import ClaudeMentionPolicy
        ...
        raise click.ClickException(f"Failed to install hooks: {e}")
...
def uninstall_hooks(project_path: str, force: bool):
    ...
    try:
        ...
        if result.success:
            ...
            for error in result.errors:
                console.print(f"  • {error}")
            raise click.ClickException("Hook removal failed")
        
    except Exception as e:
        console.print(f"[red]Error removing hooks: {e}[/red]")
        raise click.ClickException(f"Failed to remove hooks: {e}")
...
def list_backups(project_path: str, output_format: str):
    ...
    try:
        ...
        raise click.ClickException(f"Failed to list backups: {e}")
