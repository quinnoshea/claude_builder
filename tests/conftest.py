...
"""
    ...
    (src_dir / "core.py").write_text(
        """\"\"\"Core business logic for test project.\"\"\"
...
def sample_analysis() -> ProjectAnalysis:
    ...
    return ProjectAnalysis(
        ...
        framework_info=FrameworkInfo(
            primary="fastapi", secondary=["pydantic"], confidence=85.0, version="0.95.0"
        ),
...
def assert_file_exists(file_path: Path, content_contains: str = None):
    ...
    if content_contains:
        ...
        assert (
            content_contains in content
        ), f"Content '{content_contains}' not found in {file_path}"
    ...
    for item_name, item_spec in expected_structure.items():
        ...
        if isinstance(item_spec, dict):
            ...
            if item_spec and isinstance(item_spec, str):
                ...
                assert (
                    item_spec in content
                ), f"Content '{item_spec}' not found in {item_path}"
