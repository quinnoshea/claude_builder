...
def test_generate_exception_handling(mock_agent_system, mock_template_manager, sample_analysis):
    ...
    generator.template_manager.generate_from_analysis.side_effect = Exception("Template error")
...
def test_create_template_variables_with_agent_config(sample_analysis):
    ...
    mock_agent_config.all_agents = mock_agent_config.core_agents + mock_agent_config.domain_agents
...
def test_get_template_info(mock_template_manager):
    ...
    mock_manager.list_available_templates.return_value = ["template1", "template2", "template3"]
    ...
    assert template_info["templates_available"] == ["template1", "template2", "template3"]
...
class TestTemplateLoader:
    ...
    def test_load_template_failure(self, mock_template_manager):
        ...
        with pytest.raises(GenerationError, match="Failed to load template 'missing-template'"):
