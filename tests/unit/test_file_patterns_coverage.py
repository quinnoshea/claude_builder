...
def test_is_test_file():
    ...
    assert (
        FilePatterns.is_test_file(Path("utils.js")) is False
    )  # No test indicator in filename
    assert (
        FilePatterns.is_test_file(Path("__tests__.js")) is True
    )  # test indicator in filename
...
def test_should_ignore():
    ...
    assert (
        FilePatterns.should_ignore(Path("/project/.git/config"), project_root) is True
    )
    assert (
        FilePatterns.should_ignore(
            Path("/project/node_modules/package.json"), project_root
        )
        is True
    )
    assert (
        FilePatterns.should_ignore(Path("/project/target/debug/app"), project_root)
        is True
    )
    assert (
        FilePatterns.should_ignore(
            Path("/project/__pycache__/module.pyc"), project_root
        )
        is True
    )
    assert (
        FilePatterns.should_ignore(Path("/project/.vscode/settings.json"), project_root)
        is True
    )
    ...
    assert (
        FilePatterns.should_ignore(Path("/project/src/app.js"), project_root) is False
    )
...
def test_file_pattern_matcher():
    ...
    )  # Implementation provides defaults for empty list
...
def test_pattern_rule():
    ...
    assert (
        exclude_rule.matches("test_file.py") is False
    )  # "test*" not in "test_file.py"
