...
class TestConfigCommands:
    ...
    def test_config_init_command(self, mock_config_class, sample_python_project):
        ...
        assert result.exit_code == 0
        assert "Configuration created successfully" in result.output or "initialized" in result.output.lower()

    @patch('claude_builder.cli.config_commands.ConfigManager')
    def test_config_init_command_with_format(self, mock_config_class, sample_python_project):
    ...
    @patch('claude_builder.cli.config_commands.ConfigManager')
    def test_config_show_command_specific_section(self, mock_config_class, sample_python_project):
    ...
    def test_config_validate_command_invalid(self, mock_config_class, sample_python_project):
        ...
        mock_config = Mock()
        mock_config._load_config_file.side_effect = ConfigError("Missing required field: project.name")
        mock_config_class.return_value = mock_config
    ...
    def test_config_reset_command(self, mock_config_class, sample_python_project):
        ...
        assert result.exit_code == 0
        assert "reset" in result.output.lower() or "configuration" in result.output.lower()

    @patch('claude_builder.cli.config_commands.ConfigManager')
    def test_config_reset_command_with_force(self, mock_config_class, sample_python_project):
