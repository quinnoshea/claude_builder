...
class TestGitCommands:
    ...
    def test_setup_exclude_command(
        self, mock_git_manager_class, mock_config_manager_class, sample_python_project
    ):
    ...
    def test_setup_exclude_command_with_patterns(
        self,
        mock_path_exists,
        ...
        runner = CliRunner()
        result = runner.invoke(git, ["exclude", str(sample_python_project), "--force"])
        assert result.exit_code == 0

    @patch("claude_builder.cli.git_commands.ConfigManager")
    @patch("claude_builder.cli.git_commands.GitIntegrationManager")
    def test_setup_exclude_command_dry_run(
        self, mock_git_manager_class, mock_config_manager_class, sample_python_project
    ):
    ...
    @patch("claude_builder.cli.git_commands.GitIntegrationManager")
    def test_remove_exclude_command(
        self, mock_git_manager_class, sample_python_project
    ):
    ...
    @patch("claude_builder.cli.git_commands.GitIntegrationManager")
    def test_remove_exclude_command_with_backup(
        self, mock_git_manager_class, sample_python_project
    ):
        """Test git remove-exclude command with backup."""
        mock_git_manager = Mock()
        mock_git_manager.remove_git_exclude.return_value = Mock(
            success=True, files_removed=["CLAUDE.md"], backup_created="exclude.backup"
        )
        mock_git_manager_class.return_value = mock_git_manager
    ...
    @patch("claude_builder.cli.git_commands.GitIntegrationManager")
    def test_git_status_command_non_git_repo(
        self, mock_git_manager_class, sample_python_project
    ):
        """Test git status command for non-git repository."""
        mock_git_manager = Mock()
        mock_git_manager.get_git_status.return_value = Mock(
            is_git_repo=False, error_message="Not a git repository"
        )
        mock_git_manager_class.return_value = mock_git_manager
    ...
    @patch("claude_builder.cli.git_commands.GitBackupManager")
    def test_backup_command_with_name(
        self, mock_backup_manager_class, sample_python_project
    ):
        ...
        result = runner.invoke(
            git, ["list-backups", str(sample_python_project), "--format", "json"]
        )
        assert result.exit_code == 0
    ...
    def test_restore_command(self, mock_backup_manager_class, sample_python_project):
        ...
        runner = CliRunner()
        result = runner.invoke(git, ["rollback", "backup1", str(sample_python_project)])
        assert result.exit_code == 0

    @patch("claude_builder.cli.git_commands.GitBackupManager")
    def test_restore_command_with_confirmation(
        self, mock_backup_manager_class, sample_python_project
    ):
        """Test git cleanup-backups command."""
        mock_backup_manager = Mock()
        mock_backup_manager.cleanup_backups.return_value = Mock(
            success=True, backups_removed=2
        )
        mock_backup_manager_class.return_value = mock_backup_manager

        runner = CliRunner()
        result = runner.invoke(
            git, ["cleanup-backups", str(sample_python_project), "--keep", "3"]
        )
        assert result.exit_code == 0
    ...
    @patch("claude_builder.cli.git_commands.GitIntegrationManager")
    @patch("pathlib.Path.exists")
    def test_setup_exclude_command_failure(
        self,
        mock_path_exists,
        mock_git_manager_class,
        mock_config_manager_class,
    ...
    @patch("pathlib.Path.exists")
    def test_git_command_exception_handling(
        self, mock_path_exists, sample_python_project
    ):
    ...
    def test_restore_command_missing_backup_file(
        self, mock_path_exists, mock_backup_manager_class, sample_python_project
    ):
        ...
        result = runner.invoke(
            git,
            ["rollback", "nonexistent-backup", str(sample_python_project), "--force"],
        )
        assert result.exit_code != 0
