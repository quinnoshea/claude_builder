...
class TestAnalyzeCommands:
    ...
    def test_project_command_success(
        self, mock_analyzer_class, sample_python_project, mock_analysis
    ):
    ...
    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_json_output(
        self, mock_analyzer_class, sample_python_project, mock_analysis
    ):
        ...
        result = runner.invoke(
            project, [str(sample_python_project), "--format", "json"]
        )
        assert result.exit_code == 0
    ...
    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_with_output_file(
        self, mock_analyzer_class, sample_python_project, mock_analysis, tmp_path
    ):
        ...
        runner = CliRunner()
        result = runner.invoke(
            project, [str(sample_python_project), "--output", str(output_file)]
        )
        assert result.exit_code == 0
    ...
    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_confidence_threshold(
        self, mock_analyzer_class, sample_python_project, mock_analysis
    ):
        ...
        runner = CliRunner()
        result = runner.invoke(
            project, [str(sample_python_project), "--confidence-threshold", "50"]
        )
        assert result.exit_code == 0
        assert "below threshold" in result.output

    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_verbose(
        self, mock_analyzer_class, sample_python_project, mock_analysis
    ):
        ...
        runner = CliRunner()
        result = runner.invoke(project, [str(sample_python_project), "--verbose"])
        assert result.exit_code == 0
        assert "Analyzing project at" in result.output

    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_include_suggestions(
        self, mock_analyzer_class, sample_python_project, mock_analysis
    ):
        ...
        runner = CliRunner()
        result = runner.invoke(
            project, [str(sample_python_project), "--include-suggestions"]
        )
        assert result.exit_code == 0
    ...
    @patch("claude_builder.cli.analyze_commands.ProjectAnalyzer")
    def test_project_command_analyzer_exception(
        self, mock_analyzer_class, sample_python_project
    ):
