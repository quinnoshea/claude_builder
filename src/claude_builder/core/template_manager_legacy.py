    ...
    def create_template(
        ...
        *,
        ...
        if name is None or content is None:
            msg = "name and content or config are required"
...
class TemplateRepository:
    ...
    def add_template(self, template_file: Path, *, validate: bool = False) -> None:
        ...
        if validate:
            ...
            for r in required:
                if not meta.get(r):
                    msg = f"Missing required field: {r}"
                    raise ValueError(msg)
    ...
    def __init__(self, template_engine: str = "simple", *, enable_cache: bool = False):
        ...
        if self.template_engine == "jinja2":
            try:
                ...
                msg = "Jinja2 is required for template rendering. Please install the 'jinja2' package."
        ...
        **kwargs: Any,
        ...
        if self._jinja_env is not None:
            try:
                ...
                msg = f"Failed to render template {template.name}: {e}"
                raise TemplateError(
                    msg,
