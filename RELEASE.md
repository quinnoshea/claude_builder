...
The publish workflow (`.github/workflows/publish.yml`) runs the same preflight policy
via `scripts/release_preflight.sh`.

Publish is blocked unless all of the following pass:

- Lint/type/security/test checks from the merge gates above.
- Package version consistency (`src/claude_builder/__init__.py` and `pyproject.toml`).
...
- Prioritize de-quarantining high-impact suites (CLI, generation, config).
